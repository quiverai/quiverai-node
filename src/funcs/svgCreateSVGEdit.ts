...
export enum CreateSVGEditAcceptEnum {
 ...
 */
export function svgCreateSVGEdit(
  client: QuiverAICore,
  request: shared.EditSVGRequest,
  options?: RequestOptions & { acceptHeaderOverride?: CreateSVGEditAcceptEnum },
): APIPromise<
  Result<
    operations.CreateSVGEditResponse,
    | QuiverAiError
...
> {
  const parsed = safeParse(
    request,
    (value) => shared.EditSVGRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/svg/edits")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateSVGEditResponse$inboundSchema),
    M.sse(200, operations.CreateSVGEditResponse$inboundSchema),
    M.json([400, 401, 429], operations.CreateSVGEditResponse$inboundSchema),
  )(response, req);
  if (!result.ok) {
